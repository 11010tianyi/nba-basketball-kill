...
人数配置会随 4-8 人自动调整挑战者、队友和独狼数量。每局开局会随机分配身份，玩家不一定总是核心；核心身份公开，其他身份只对自己可见。核心阵营清除全部挑战者和独狼后获胜；挑战者让核心离场后获胜；独狼需要在残局中单独取胜。
...
1. 自动开球：哨声后中圈跳球，随机一名存活玩家获得球权并成为本局首攻
2. 准备阶段：刷新本回合技能状态
3. 摸牌阶段：摸 2 张
4. 出牌阶段：使用进攻牌、战术牌、治疗牌、装备牌
5. 响应链：进攻或部分战术牌会要求目标响应
6. 弃牌阶段：手牌数超过当前体能时手动弃牌，战术板可让上限 +1
7. 结束阶段：轮到下一名存活玩家

终场后会弹出比赛结果，可以返回首页、按当前设置重开，或随机人数和球星重新开局。
...
在 `styles.css` 的 `:root` 中修改这些变量即可：

```css
--selected-card-border-color: #ff5a4f;
--selected-card-border-width: 3px;
--active-player-border-color: #ffd05a;
--active-player-border-width: 3px;
```

其中 selected 是选中手牌的红色边框，active-player 是当前出牌人物牌的黄色边框。
